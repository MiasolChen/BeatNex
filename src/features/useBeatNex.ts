...
export function useBeatNex(){
 const [settings,setSettings]=useState(readPractice)
 ...
 const reset=()=>{audio.stop();setCompleted(false);setFeedback('')}
 const switchPage=(next:Page)=>{if(next===page)return;audio.stop();setPage(next);if(next!=='machine')setLandscape(false)}
 const changeBpm=(bpm:number)=>patch({bpm})
 const togglePlayback=()=>{if(playing)audio.pause();else{if(completed)reset();void audio.play()}}
 const selectDrum=(drum:DrumId,remove:boolean)=>{patch({targets:remove?settings.targets.filter(d=>d!==drum):Array.from(new Set([...settings.targets,drum]))})}
 const choosePattern=(id:string)=>{const next=BOOM_BAP_PATTERNS.find(p=>p.id===id);if(!next)return;reset();meterDrafts.current={};setPattern(next);setSource(next);setActiveId(undefined);setMuted([]);setHistory([]);setFuture([]);patch({bpm:next.recommendedBpm,patternName:next.name,difficulty:next.difficulty});setPage('practice')}
 ...
 const changeMeter=(meter:Meter)=>{if(meter===meterForPattern(pattern))return;reset();meterDrafts.current[meterForPattern(pattern)]=pattern;changePattern(meterDrafts.current[meter]??withMeter(pattern,meter))}
 ...
 const save=(name:string,practice:boolean)=>{
  ...
  setCombinations({value:entries,error:null});setActiveId(entry.id);setPattern(entry.pattern);setSource(entry.pattern);setSaveOpen(false);notice('已保存到节奏库');if(practice){reset();patch({targets:DRUM_IDS.filter(d=>!muted.includes(d))});setPage('practice')}return true
 }
 const load=(id:string,destination:Page)=>{const entry=combinations.value.find(x=>x.id===id);if(!entry)return;reset();meterDrafts.current={};setPattern(entry.pattern);setSource(entry.pattern);setActiveId(id);setMuted(entry.muted);setHistory([]);setFuture([]);patch({bpm:entry.bpm,targets:DRUM_IDS.filter(d=>!entry.muted.includes(d))});setPage(destination);notice(destination==='practice'?'已使用 '+entry.name:'已打开 '+entry.name)}
