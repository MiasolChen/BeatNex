...
export function readTrainingConfig(storage?: Pick<Storage, 'getItem'>): TrainingConfig {
  ...
  try {
    ...
    if (value.version !== 1 || !DRUM_IDS.includes(value.targetDrum as DrumId) || !validPhases || !PATTERN_NAMES.includes(value.patternName as never) || !['simple', 'medium', 'hard'].includes(value.difficulty ?? '') || !Number.isInteger(value.bpm) || value.bpm! < 60 || value.bpm! > 140) throw new Error('invalid')
