import { DRUM_IDS, type Pattern } from './types'
...
export function validatePattern(value: unknown): asserts value is Pattern {
  ...
  if (!pattern.id || !pattern.name) throw new Error('Pattern 缺少 id 或 name')
  if (pattern.style !== 'boom-bap') throw new Error('Prototype 仅支持 boom-bap')
  ...
  if (pattern.beatsPerBar !== 4 || ![2, 3, 4].includes(pattern.subdivision ?? 0)) {
  ...
  const maxStep = (pattern.bars ?? 0) * 4 * (pattern.subdivision ?? 0)
