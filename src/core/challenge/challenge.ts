...
export function challengePosition(step: number, rounds: number, countIn = false) {
  const intro = countIn ? 16 : 0
  const total = intro + rounds * 32
  const bounded = Math.max(0, Math.min(step, total))
  return {
    complete: bounded >= total,
    countIn: bounded < intro,
    round: Math.min(rounds, Math.floor(Math.max(0, bounded - intro) / 32) + 1),
    phraseStep: Math.max(0, bounded - intro) % 32,
    step: bounded, total,
  }
}
export function challengeEvents(step: number, rounds: number, hits: readonly number[], _legacyReference: boolean, countIn = false) {
  const intro = countIn ? 16 : 0
  return {target: step >= intro && step < intro + rounds * 32 && hits.includes((step - intro) % 32), reference: countIn && step >= 0 && step < 16 && step % 4 === 0}
}
