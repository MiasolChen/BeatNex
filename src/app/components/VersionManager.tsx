...
export function VersionManager({app}:{app:BeatNex}) {
 ...
 {purge?<><p>永久删除“{purge.name}”？此操作不可恢复。</p><div className="bn-version-actions"><button onClick={()=>setPurge(null)}>取消</button><button className="bn-danger" onClick={()=>action(purge.id,'purge',purge.revisionId)}>确认永久删除</button></div></>
 ...
 :panel.trash?<>{!trash.length&&<p className="bn-muted">回收站为空</p>}{trash.map(x=><div className="bn-version-entry" key={x.id+':'+(x.revisionId??'')}><strong>{x.name}</strong><div className="bn-version-actions"><button onClick={()=>action(x.id,'restore',x.revisionId)}>恢复</button><button className="bn-danger" onClick={()=>setPurge(x)}>永久删除</button></div></div>)}</>
