...
export function RhythmChallenge({state}: {state: ChallengeState}) {
  ...
  const descriptions=['记住这段节奏，下一遍再跟着拍。','每听到一下，就拍一下手或轻敲桌面。','按刚才记住的节奏继续拍手；屏幕上的拍点会继续走。','示范声回来了，听听你的拍手是否和它重合。']
  ...
  return <section className="rc-workspace" aria-label="节奏挑战练习">
    ...
    {!ready&&!complete&&<div className="rc-silent-beats" role="img" aria-label="无声拍点：每组四拍，声音停下时也继续走"><span>无声拍点</span>{[1,2,3,4].map(n=><b key={n} data-current={status==='playing'&&Math.floor((position.countIn?position.step:position.phraseStep%16)/4)===n-1}>{n}</b>)}</div>}
    ...
    <details className="rc-next"><summary>看节奏图（可选）</summary><p>圆点表示拍一下，横线表示安静等待。空拍没有声音，指针仍会继续走。</p><Phrase challenge={challenge} step={position.phraseStep} active={status==='playing'&&!position.countIn}/></details>
    </article>
