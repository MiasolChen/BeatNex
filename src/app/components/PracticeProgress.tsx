...
export function PracticeProgress({ total, cycle, snapshot, complete, enabled, guide, round = 1, infinite = false, phaseLabel }: {
  ...
  const started = !snapshot.isCountIn && (snapshot.status === 'playing' || snapshot.status === 'paused' || snapshot.elapsed > 0)
  const { done, current, start, end } = barProgress(total, cycle, started, complete)
  const label = snapshot.isCountIn ? '预备拍 · 准备听示范' : complete ? `已完成 ${total} / ${total} 小节` : started ? `${infinite ? `第 ${round} 轮 · ` : ''}第 ${current} / ${total} 小节` : `准备开始 · 共 ${total} 小节`
  return <article className="bn-card bn-practice-progress" hidden={!enabled} aria-label="小节进度">
    ...
    {phaseLabel && <p className="bn-current-phase">{complete?'练习完成':started?'当前阶段':'首个阶段'} · {phaseLabel}</p>}
