...
function mark(d:string,x:number,y:number,scale=1,color='var(--bn-secondary)',width=2,opacity=1,parent:Element=content){const g=node('g',{transform:`translate(${x} ${y}) scale(${scale})`,opacity},parent);node('path',{d,stroke:color,'stroke-width':width},g);return g}
// Header: one directional blue gesture, centered in the existing gap.
const brand=bounds('.bn-brand'),local=bounds('.bn-local');if(brand&&local){const gap=local.x-brand.right;if(gap>25){const w=Math.min(33,gap-13),x=brand.right+(gap-w)/2,y=brand.y+brand.h/2-7;mark('M1 12 25 3 M8 16 32 6 15 8 M28 0 31 3',x,y,w/33,'var(--bn-secondary)',2.1,1,art)}}
...
const metro=bounds('.bn-metronome'),nav=bounds('#bn>.bn-nav');if(metro&&nav){const free=nav.y-metro.bottom;if(free>88){const scale=Math.min(1.1,(free-34)/78);shoes(base.width/2-55*scale,metro.bottom+(free-74*scale)/2,scale);if(free>150)mark('M0 14 16 14 22 1 29 27 38 7 45 17 60 17',base.width/2-28,metro.bottom+(free-90)/2+85,.85,'var(--bn-secondary)',1.8,.65)}}
...
mark('M8 47C-3 26 31-1 66 4S103 33 80 47 20 60 13 39C9 25 29 12 44 10 M20 57 79 15 47 68 95 40 M4 66l-9 4',left+8,top+16,1.38,'var(--bn-muted)',2.8,.62,stageMarks);
// A small torn-paper fragment follows the loop down and right, rather than facing a twin.
mark('M4 4 49 0 46 12 54 17 49 45 7 49 10 38 1 32 4 4 M11 11l23-3 M12 18l31-5 M15 25l18-3 M20 32l21-4 M3 56 33 51',left+stage.w*.54,top+104,.66,'var(--bn-secondary)',1.9,.8,stageMarks);
// Sparse high scratches extend the upper cluster in one direction.
mark('M0 14 38 4 M12 21 66 6 M56 1l8-2 M71 8l2-1',left+stage.w*.53,top+19,.92,'var(--bn-muted)',1.7,.72,stageMarks);
// A compact cross sits at a different height, followed by a long broken downstroke.
mark('M4 1 36 38 M0 36 35 4 M8 2 40 32 M46 6l3-2 M52 13l2 1',right-49,top+77,.5,'var(--bn-secondary)',2.1,.8,stageMarks);
mark('M4 0 0 24 6 29 5 59 M12 34l-3 15',right-21,top+114,.72,'var(--bn-muted)',1.8,.7,stageMarks);
// Only one grounded scuff; its longer horizontal tail anchors the right-hand weight.
mark('M0 13 9 1 16 17 26 3 M-3 24l35-5 M18 29l43-7 M38 6l3-8',right-67,ground+12,.74,'var(--bn-secondary)',2,.76,stageMarks);
}
...
mark('M6 18 111 1Q120 0 121 10L131 74Q132 83 122 85L18 101Q8 102 7 92L0 30Q-1 21 6 18Z M9 29 109 13 118 68 17 84 9 29 M28 35 94 24 101 57 35 68 28 35 M39 46c-8-5-11 8-3 11s13-8 3-11 M84 37c-8-5-11 8-3 11s13-8 3-11 M43 49l32-5 M39 76l3-11 43-7 7 10 M16 94l6-1 M114 78l5-1',x,y,s,'var(--bn-muted)',1.7,.85);
mark('M114 22C137 8 148 17 142 36S135 64 154 57 172 41 181 47',x,y,s,'var(--bn-muted)',2,.8);
mark('M5 115l43-6 M158 20l12-5',x,y,s,'var(--bn-muted)',1.6,.7);
}}
// A small hand-drawn page curl at the calendar's lower free corner.
const sheet=bounds('.bn-calendar-sheet');if(sheet){mark('M0 0q-2 12-15 15 M-14 15q9-1 13-3l1-12',sheet.right-7,sheet.bottom-19,1,'var(--bn-muted)',1.7,.75)}
}
